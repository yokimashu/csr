...
include('../config/db_config.php');
 ...
 if(isset($_POST['semester'])){
    ...
    $sql = "SELECT * FROM `tbl_enrollment_item` WHERE objid = :objid";
    $prep_sql = $con->prepare($sql);
    $prep_sql->execute([':objid'=> $objid
    ]);

    

    while($result = $prep_sql->fetch(PDO::FETCH_ASSOC)){
        $subject_course = $result['subject_code'];
        $description = $result['descriptive_title'];
        $units = $result['units'];
        $days = $result['days'];
        $time = $result['time'];
        $room = $result['room'];
